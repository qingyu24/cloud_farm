...
public class FriendService {
    ...
    public Boolean agree(ResultInfo resultInfo, Friend friend) {
        ...
        HashMap<String, Object> data = resultInfo.getData();
        List<FriendInfo> m_friends = new ArrayList<>(2);

        FriendInfo friendInfo=player.getOneFriendinfo(friend.getFriendid());
        if (friendInfo!=null)
        m_friends.add(friendInfo);
        data.put("friends", m_friends);
        return true;
    }
    ...
    public List<Friend> getfriend(Friend friend) {
        ...
        for (Friend friend1 : friends) {
            if (friend1.getFriendid().longValue() == friend.getFriendid().longValue() && friend1.getUserid().longValue() == friend.getUserid().longValue()) {
        ...
        for (Friend friend1 : friends1) {
            if (friend1.getFriendid().longValue() == friend.getUserid().longValue() && friend1.getUserid().longValue() == friend.getFriendid().longValue()) {
                m_friends.add(friend1);
            }
        }
        return m_friends;
    ...
    public boolean searchOne(ResultInfo resultInfo, Friend friend) {
        Long friendid = friend.getFriendid();
        PlayerInfo player = PlayerManager.getPlayer(friendid);
        FriendInfo ownerInfo = player.getOwnerInfo();
        if(ownerInfo==null){
            resultInfo.setResp_code("113001");
            return false;
        }
        ownerInfo.setFriendid(ownerInfo.getUserid());
        ownerInfo.setUserid(friend.getUserid());
        ArrayList<FriendInfo> friendInfos = new ArrayList<>(1);
        if (ownerInfo != null) friendInfos.add(ownerInfo);
        resultInfo.getData().put("friends", friendInfos);
        return true;
    }
}
