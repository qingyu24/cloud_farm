...
import com.fngame.farm.configer.WareHouse;
...
public class StreetMarketService {
    ...
    public boolean removeGoods(ResultInfo resultInfo, Long userid, Long id) {
        ...
        Integer type=streetMarket.getFlag();
        Integer propslevel;
        //获取玩家仓库等级
        User user=playerInfo.getUser();
        Integer aaa=playerInfo.getUser().getMarketCapacity();
        if(type.equals( EPropType.CROPS.ID())){
            propslevel=playerInfo.getUser().getGranaryLevel();//当前玩家的粮仓等级
        }else{
            propslevel=playerInfo.getUser().getPropsLevel();//当前玩家的货仓等级
        }

        //根据等级获取仓库容量
        WareHouse wareHouse = ConfigManager.getInstance().getWareHouseByLevel(propslevel);
        Integer maxHouseCapacity = wareHouse.Capacity;
        Integer getTotalCapacity = this.getTotalCapacity(userid,type);
        if(getTotalCapacity+streetMarket.getOnshelfnum()>maxHouseCapacity){//判断仓库容量是否达上限
            resultInfo.setResp_code("600024");
            return false;
        }
    ...
    * 获得玩家当前仓库的容量
    * */
    public Integer getTotalCapacity(Long userid,Integer type){
        PlayerInfo playerInfo = PlayerManager.getPlayer(userid);
        //累加玩家对应仓库的总数
        List<Props> propsList = playerInfo.getWareHouse(userid,type);
        int count=0;
