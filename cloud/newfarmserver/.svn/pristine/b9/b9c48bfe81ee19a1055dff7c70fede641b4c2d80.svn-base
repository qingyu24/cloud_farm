...
public class BuildingController extends BaseController implements BaseContorllerInterface<Building> {
    ...
    public ResultInfo add(RequserOrder order, Building building) {
      ...
      resultInfo=new ResultInfo();
        ...
        boolean b = false;
        try {
            b = BuildingService.add(this.resultInfo, building);
        } catch (Exception e) {
            e.printStackTrace();
        }
        System.out.println(this.resultInfo.toString());
        if (b) {
            this.resultInfo.setSucess(true);
            this.resultInfo.getData().put("buildinfo", building);
            ...
            if (null == playerInfo){
                //返回玩家不存在的错误;
                this.resultInfo.setResp_code("300001");
                return this.resultInfo;
            }
            Building build = playerInfo.getBuildingByID(building.getId());
            if (null == build) {
                //没有对应的目标建筑物;
                this.resultInfo.setResp_code("200004");
                return this.resultInfo;
            }
